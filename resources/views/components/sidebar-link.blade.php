@props(['active' => false, 'icon' => 'circle', 'href' => '#', 'badge' => null])

@php
$classes = $active
    ? 'bg-gradient-to-r from-indigo-600 to-blue-600 text-white shadow-lg shadow-indigo-500/20 font-semibold border-l-2 border-cyan-400'
...
<a href="{{ $href }}" class="flex items-center justify-between px-3.5 py-2.5 rounded-xl text-xs sm:text-sm transition-all duration-150 group {{ $classes }}">
    <div class="flex items-center gap-3 min-w-0">
        <svg class="w-4 h-4 shrink-0 transition-transform group-hover:scale-110 {{ $active ? 'text-white' : 'text-slate-400 group-hover:text-indigo-400' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24">{!! $svgPath !!}</svg>
        <span class="truncate">{{ $slot }}</span>
    </div>
    @if($badge)
        <span class="px-2 py-0.5 text-[10px] font-bold rounded-full {{ $active ? 'bg-white/20 text-white' : 'bg-slate-800 text-slate-400' }}">{{ $badge }}</span>
    @endif
</a>
