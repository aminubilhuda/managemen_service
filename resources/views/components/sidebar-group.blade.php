@props(['title', 'icon' => 'circle', 'active' => false, 'createRoute' => null, 'createTitle' => 'Tambah Baru'])
...
$icons = [
    ...
    'percent' => '<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 7.5a1.5 1.5 0 110 3 1.5 1.5 0 010-3zm6 6a1.5 1.5 0 110 3 1.5 1.5 0 010-3zM5.5 18.5l13-13"/>',
    ...
    'circle' => '<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"/>',
];
...
<div x-data="{ open: {{ $active ? 'true' : 'false' }} }" class="space-y-1">
    <div class="flex items-center justify-between rounded-xl transition-all duration-150 {{ $active ? 'bg-slate-900/90 text-white shadow-sm border-l-2 border-indigo-500' : 'text-slate-400 hover:text-slate-100 hover:bg-slate-900/50' }}">
        <button @click="open = !open" type="button" class="flex-1 flex items-center justify-between px-3 py-2 text-xs sm:text-sm font-medium group text-left">
            <div class="flex items-center gap-2.5 min-w-0">
                <svg class="w-4 h-4 shrink-0 transition-transform group-hover:scale-110 {{ $active ? 'text-indigo-400' : 'text-slate-400 group-hover:text-indigo-400' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    {!! $svgPath !!}
                </svg>
                <span class="truncate font-semibold">{{ $title }}</span>
            </div>
            <svg :class="open ? 'rotate-90 text-indigo-400' : 'text-slate-600'" class="w-3.5 h-3.5 transform transition-transform duration-200 shrink-0 ml-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"/>
            </svg>
        </button>

        @if($createRoute)
            <a href="{{ $createRoute }}" title="{{ $createTitle }}" class="p-1.5 mr-1 text-slate-500 hover:text-cyan-400 hover:bg-slate-800 rounded-lg transition" @click.stop>
    ...
    <div x-show="open" 
         ...
         class="ml-5 pl-2 space-y-0.5 border-l border-slate-800/80 my-1">
        {{ $slot }}
    </div>
</div>
