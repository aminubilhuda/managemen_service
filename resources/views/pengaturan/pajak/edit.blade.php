<x-app-layout>
    <x-slot name="title">Edit Tarif Pajak</x-slot>
    <x-slot name="header">Edit Tarif Pajak</x-slot>
    <x-slot name="subtitle">Perbarui nilai persentase tarif PPN</x-slot>

    <div class="max-w-2xl">
        <div class="bg-white rounded-2xl border border-slate-200/80 p-6 sm:p-8 shadow-sm">
            <form method="POST" action="{{ route('pengaturan.pajak.update', $pajak) }}" class="space-y-6">
                @csrf
                @method('PUT')
                ...
                <div>
                    ...
                    <input type="text" name="nama_pajak" id="nama_pajak" value="{{ old('nama_pajak', $pajak->nama_pajak) }}" required
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                    <div>
                        ...
                        <div class="relative">
                            <input type="number" name="persentase" id="persentase" value="{{ old('persentase', $pajak->persentase) }}" required min="0" max="100" step="0.1"
                    ...
                    <div>
                        ...
                        <input type="date" name="berlaku_mulai" id="berlaku_mulai" value="{{ old('berlaku_mulai', $pajak->berlaku_mulai?->format('Y-m-d')) }}" required
                               class="w-full rounded-xl border border-slate-200 px-4 py-2.5 text-sm focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/20 transition">
                    </div>
                </div>

                <div x-data="{ tipeAturan: '{{ old('tipe_aturan', $pajak->tipe_aturan ?? 'semua') }}' }" class="space-y-4">
                    <div>
                    ...
                    <div x-show="tipeAturan !== 'semua'" x-cloak class="p-4 rounded-xl bg-slate-50 border border-slate-200 space-y-2">
                        ...
                        <div class="relative">
                            ...
                            <input type="number" name="nominal_batas" id="nominal_batas" value="{{ old('nominal_batas', (int) $pajak->nominal_batas ?: 2000000) }}" min="0" step="50000"
                                   placeholder="2000000"
                ...
                <div>
                    <label class="inline-flex items-center gap-2 cursor-pointer">
                        <input type="checkbox" name="aktif" value="1" {{ old('aktif', $pajak->aktif) ? 'checked' : '' }} class="rounded border-slate-300 text-indigo-600 focus:ring-indigo-500">
                ...
                <div class="pt-4 border-t border-slate-100 flex items-center justify-between">
                    <button type="button" onclick="if(confirm('Yakin ingin menghapus tarif pajak ini?')) document.getElementById('delete-pajak-form').submit();" class="px-4 py-2.5 rounded-xl text-rose-600 hover:bg-rose-50 text-xs sm:text-sm font-semibold transition inline-flex items-center gap-1.5">
            ...
            <form id="delete-pajak-form" method="POST" action="{{ route('pengaturan.pajak.destroy', $pajak) }}" class="hidden">
                @csrf
                @method('DELETE')
            </form>
        </div>
