<x-app-layout>
    ...
    <x-slot name="header">Tambah Pelanggan Baru</x-slot>
    <x-slot name="subtitle">Daftarkan data identitas dan kontak pelanggan untuk servis atau pembelian</x-slot>
        ...
        <div class="bg-white rounded-2xl border border-slate-200/80 p-6 sm:p-8 shadow-sm">
            <form method="POST" action="{{ route('pelanggan.store') }}" class="space-y-6">
                ...
                <div>
                    ...
                    <input type="text" name="nama_pelanggan" id="nama_pelanggan" value="{{ old('nama_pelanggan') }}" required
                ...
                <div>
                    ...
                    <textarea name="alamat" id="alamat" rows="3"
                              placeholder="Jl. Merdeka No. 45, RT 02/03..."
                              class="w-full rounded-xl border border-slate-200 px-4 py-2.5 text-sm focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/20 transition @error('alamat') border-rose-500 @enderror">{{ old('alamat') }}</textarea>
                    @error('alamat')
                        <p class="text-xs text-rose-500 mt-1.5">{{ $message }}</p>
                    @enderror
                </div>
